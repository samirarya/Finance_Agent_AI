...
# --- LLM Configurations ---
PRIMARY_LLM = "gpt-4o"
SECONDARY_LLM = "gemini/gemini-2.5-flash"
...
def run_sammy_workflow(user_query):
    ...
    print(f"--- Tier 1: Attempting OpenAI Workflow ({PRIMARY_LLM}) ---")
    try:
        ...
        error_str = str(e)
        if any(kw in error_str for kw in ["429", "RESOURCE_EXHAUSTED", "quota", "insufficient_quota"]):
            # 2. Fallback to Gemini (Secondary)
            print(f"⚠️ OpenAI limit hit. Tier 2: Falling back to Gemini ({SECONDARY_LLM})...")
            time.sleep(1)
            try:
                # Gemini is capable enough for hierarchy
                crew = create_hierarchical_crew(user_query, SECONDARY_LLM)
                result = crew.kickoff()
                return "*(Note: This response was generated by Gemini as a secondary fallback)*\n\n" + str(result)
            except Exception as e2:
                # 3. Fallback to Local Llama (Tertiary)
                print(f"⚠️ Gemini also hit an issue: {e2}. Tier 3: Falling back to Local Llama ({TERTIARY_LLM})...")
                time.sleep(1)
                try:
